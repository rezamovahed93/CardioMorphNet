...
def Visualize_SeqOrd_frame_overlay(image_data, mask_data, delay=0.5, cmap='gray', alpha=0.5):
    """
    Visualizes image slices with an overlayed mask.

    Parameters:
        image_data (numpy.ndarray): 3D array of the image data (height, width, slices).
        mask_data (numpy.ndarray): 3D array of the mask data (height, width, slices).
        delay (float): Delay between displaying slices in seconds.
        cmap (str): Colormap for the image.
        alpha (float): Transparency of the mask overlay (0 = fully transparent, 1 = fully opaque).
    """
    for i in range(image_data.shape[-1]):
        ...
        # Overlay the mask slice
        mask_slice = mask_data[:, :, i]
        ax.imshow(mask_slice, cmap='jet', alpha=alpha)  # Use 'jet' colormap for the mask
        
        ax.set_title(f'Slice {i + 1} with Overlayed Mask')
        ax.axis('off')
...
def Visualize_SeqOrd(image_data, delay=0.5, cmap='gray'):
    for i in range(image_data.shape[2]):
        ...
        image_slice = image_data[:, :, i]
        axes1.imshow(image_slice, cmap='gray', vmin=0, vmax=1)
...
def all_visulize_with_gt(img_target, img_moved, predicted_mask, 
                 pre_moved_mask, mask_org, moved_mask, displacement, 
    ...
    mask_org = mask_org[:, :, : ,: , slice_dx]
    moved_mask = moved_mask[:, :, : ,: , slice_dx]
    pre_moved_mask = pre_moved_mask[:, :, : ,: , slice_dx]
    jacobian = jacobian[:, :, :, slice_dx]
    displacement = displacement[:,:,:,:,slice_dx]
    
    norm_jac = TwoSlopeNorm(vmin=-2, vcenter=0, vmax=2)
    ...
    fig, axs = plt.subplots(9, frame_size, figsize=(19, 21))
    ...
    for i in range(frame_size):
        ...
        ax = axs[3,i]
        ax.imshow(np.argmax(predicted_mask[i], axis=0), cmap=cmap_seg)
        ...
        ax = axs[4,i]
        ax.imshow(np.argmax(pre_moved_mask[i], axis=0), cmap=cmap_seg)
        ...
        ax = axs[5,i]
        ax.imshow(np.argmax(moved_mask[i], axis=0), cmap=cmap_seg)
        
        if i == 0:
            ax.set_ylabel("Moved Mask", fontsize=fontsize)
        ...
        ax = axs[7,i]
        pcm = ax.imshow(jacobian[i], cmap=plt.cm.RdBu_r, norm=norm_jac)
        ...
        if i == 0 or i ==3:
            ax.imshow(np.argmax(mask_org[i], axis=0), cmap=cmap_seg)
...
def all_visulize(img_target, img_moved, predicted_mask, 
...
def all_visulize_seg(img_target, predicted_mask, ground_truth, 
    ...
    ground_truth = ground_truth[ES_ED_idx, :, : ,: , slice_dx]
    ...
    for i in range(frame_size):
        ...
        ax = axs[1,i]
        ax.imshow(np.argmax(ground_truth[i], axis=0), cmap=cmap_seg)
...
def all_visulize_seg_only(img_target, predicted_mask, ground_truth, 
                 slice_dx, saving_dir, label_title, format='png'):
    ...
    fig, axs = plt.subplots(3, frame_size, figsize=(19,12.5))
    fig.suptitle(label_title, fontweight="bold", fontsize= 14)
...
def saving_results_per_cat(img_target, img_moved, predicted_mask, moved_mask, 
                           displacement, jacobian, mask_org, es_ed_idxs, slice_dx, saving_lik):

    create_directory(os.path.join(saving_lik, 'results_fig'))
    saving_lik = os.path.join(saving_lik, 'results_fig')
    create_directory(os.path.join(saving_lik,'Jac'))
    create_directory(os.path.join(saving_lik,'DVF'))
    create_directory(os.path.join(saving_lik,'Img'))
    create_directory(os.path.join(saving_lik,'Pre Img'))
    create_directory(os.path.join(saving_lik,'Pre Mask'))
    create_directory(os.path.join(saving_lik,'Moved Pre Mask'))
    create_directory(os.path.join(saving_lik,'Abs err'))
    create_directory(os.path.join(saving_lik,'Mask org'))
    ...
    for i in range(frame_size):
        ...
        plt.imshow(np.argmax(predicted_mask[i], axis=0), cmap=cmap_seg)
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        ...
        plt.imshow(np.argmax(moved_mask[i], axis=0), cmap=cmap_seg)
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        ...
        plt.imshow(jacobian[i], cmap=plt.cm.RdBu_r, norm=norm_jac)
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
    ...
    for i in es_ed_idxs:
        each_mask_org = mask_org[i, :, :, :]
        # plt.imshow(each_mask_org[[1,2,3],:,:].transpose((1,2,0)), cmap=cmap_seg)
        plt.imshow(np.argmax(mask_org[i, :, :, :], axis=0), cmap=cmap_seg)
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        
        plt.savefig(os.path.join(saving_lik,'Mask org', f"I{i}.png"),  bbox_inches='tight')
    ...
    sm = plt.cm.ScalarMappable(cmap=plt.cm.RdBu_r, norm=norm_jac)
    sm.set_array([])
...
# def all_visulize_gen(img_target, img_gen, predicted_mask, displacement, 
#                      jacobian, slice_dx, saving_dir, format='png',
#                      label_title= "Registration", 
#                      label_out= "Registred"):
...
#     fig, axs = plt.subplots(5, frame_size, figsize=(8, 6))
#     # fig.suptitle(label_title, fontweight="bold", fontsize= 14)
...
#         ax = axs[2,i]
#         ax.imshow(predicted_mask[i][[1,2,3],:,:].transpose((1,2,0)), cmap=cmap_seg)
