...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 20 10:38:32 2024
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(dataloader_val):
            ...
            es_ed_idx_list = [int(frame_idx) for frame_idx in es_ed_idx.squeeze(0)]
            seg_in_ed_es = seg_in[:, es_ed_idx_list]
            seq_in_ed_es = seq_in[:, es_ed_idx_list]
            
            seq_in = seq_in.to(device)
            seg_in = seg_in.to(device)
            
            _, seg_outs = model.forward_seg(seq_in, seg_in, es_ed_idx)
            ...
            for b in range(batch_size):
                ...
                result = {'Segs_out': seg_outs[b],
                          'ES_ED_Idxs': es_ed_idx[b].cpu().numpy(), 
                          'Segs_in': seg_in_ed_es[b].cpu().numpy(),
                          'Seqs_in': seq_in_ed_es[b].cpu().numpy()}
